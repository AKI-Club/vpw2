...
static void Usage(char* execName){
	...
	printf("  -a OUTPUT_INCLUDE    output filetable linker symbol filename (optional; default: %s)\n",defaultArgs.outLinkerFilename);
	...
	for(int i = 1; i < argc; i++){
		if(argv[i][0] == '-'){
			switch(argv[i][1]){
				...
				case 'a': /* output filetable linker include */
					if(++i >= argc){
...
int main(int argc, char* argv[]){
	...
	FILE* outInclude; /* filetable symbols for linker */
	int numFiles = 0;
	unsigned long curLocation = 0;
	int curFileLength;
	int decodedFileLength;

	printf("akifiletable v1.4 - Filetable builder for N64 AKI wrestling games\n");
	...
	outInclude = fopen(progArgs.outLinkerFilename,"w");
	if(outInclude == NULL){
